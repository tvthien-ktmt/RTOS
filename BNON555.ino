...
#define CAN_ID_IMU  0x100   // BNO055: heading, roll, pitch (×100) + cal
#define CAN_ID_DIST 0x103   // Distance in cm (uint32) + GPS status
...
float rollDeg    = 0.0;   // euler.y() — nghiêng trái/phải
float pitchDeg   = 0.0;   // euler.z() — chúi mũi/đuôi
...
//====================================================
// CAN BUS SEND
// Frame 0x100 — IMU (8 bytes):
//   [0..1] heading ×100  int16 little-endian
//   [2..3] roll    ×100  int16 little-endian
//   [4..5] pitch   ×100  int16 little-endian
//   [6]    sys_cal (0–3)
//   [7]    mag_cal (0–3)
//
// Frame 0x103 — Distance (6 bytes):
//   [0..3] distance_cm  uint32 little-endian
//   [4]    local_ok  = 1
//   [5]    remote_ok = 1
//====================================================
...
void sendCANData() {
  // ---- Frame 0x100: IMU ----
  {
    ...
    twai_message_t msg;
    msg.identifier       = CAN_ID_IMU;
    msg.extd             = 0;
    msg.rtr              = 0;
    msg.data_length_code = 8;

    msg.data[0] =  hdg        & 0xFF;
    msg.data[1] = (hdg >> 8)  & 0xFF;
    msg.data[2] =  rol        & 0xFF;
    msg.data[3] = (rol >> 8)  & 0xFF;
    msg.data[4] =  pit        & 0xFF;
    msg.data[5] = (pit >> 8)  & 0xFF;
    msg.data[6] = sysCal;
    msg.data[7] = magCal;

    if (twai_transmit(&msg, pdMS_TO_TICKS(50)) == ESP_OK) {
      Serial.println("CAN 0x100 OK");
    } else {
      Serial.println("CAN 0x100 FAIL");
    }
  }

  // ---- Frame 0x103: Distance ----
  {
    uint32_t dist_cm = (uint32_t)lroundf(distanceM * 100.0f);

    twai_message_t msg;
    msg.identifier       = CAN_ID_DIST;
    msg.extd             = 0;
    msg.rtr              = 0;
    msg.data_length_code = 6;

    msg.data[0] =  dist_cm        & 0xFF;
    msg.data[1] = (dist_cm >>  8) & 0xFF;
    msg.data[2] = (dist_cm >> 16) & 0xFF;
    msg.data[3] = (dist_cm >> 24) & 0xFF;
    msg.data[4] = 1;   // local GPS OK (GPS thuyền = simulated)
    msg.data[5] = 1;   // remote GPS OK (GPS nạn nhân = simulated)
...
void loop() {
  ...
  headingDeg = euler.x();   // Yaw:   0–360°  (hướng thuyền)
  rollDeg    = euler.y();   // Roll:  ±90°    (nghiêng ngang)
  pitchDeg   = euler.z();   // Pitch: ±180°   (chúi mũi/đuôi)
